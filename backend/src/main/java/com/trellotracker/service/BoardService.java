...
public class BoardService {
    ...
    public Board createBoard(BoardRequest request) {
        ...
        if (request.getListTitles() != null && !request.getListTitles().isEmpty()) {
            listTitles.addAll(request.getListTitles());
    ...
    /** Actualiza título, descripción, fondo y favorito del tablero. */
    public Board updateBoard(String id, BoardRequest request) {
        ...
        if (request.getBackground() != null)  board.setBackground(request.getBackground().isEmpty() ? null : request.getBackground());
        if (request.getStarred() != null)     board.setStarred(request.getStarred());
        board.setUpdatedAt(LocalDateTime.now());
        return boardRepository.save(board);
    }
    ...
    /** Renombra una lista/columna. */
    public Board renameList(String boardId, String listId, String title) {
        Board board = boardRepository.findById(boardId).orElseThrow();
        board.getLists().stream()
                .filter(bl -> bl.getId().equals(listId))
                .findFirst()
                .ifPresent(bl -> bl.setTitle(title));
        board.setUpdatedAt(LocalDateTime.now());
        return boardRepository.save(board);
    }

    /** Elimina una lista/columna con todas sus tarjetas. */
    public Board deleteList(String boardId, String listId) {
        Board board = boardRepository.findById(boardId).orElseThrow();
        board.getLists().removeIf(bl -> bl.getId().equals(listId));
        // reindexar order
        for (int i = 0; i < board.getLists().size(); i++) board.getLists().get(i).setOrder(i);
        board.setUpdatedAt(LocalDateTime.now());
        return boardRepository.save(board);
    }
    ...
    public Board updateCard(String boardId, String listId, String cardId, CardRequest request) {
        ...
        findCard(board, listId, cardId).ifPresent(card -> {
            ...
            if (request.getCover() != null)       card.setCover(request.getCover().isEmpty() ? null : request.getCover());
            if (request.getRecurring() != null)   card.setRecurring(request.getRecurring().isEmpty() ? null : request.getRecurring());
            if (request.getDone() != null) {
                card.setDone(request.getDone());
                card.setStatus(request.getDone() ? "done" : "active");
            }
            if (request.getLabels() != null)          card.setLabels(request.getLabels());
            if (request.getAssignedMembers() != null) card.setAssignedMembers(request.getAssignedMembers());
            card.setUpdatedAt(LocalDateTime.now());
        });
        board.setUpdatedAt(LocalDateTime.now());
        return boardRepository.save(board);
